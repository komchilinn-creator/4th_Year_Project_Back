...
final class Teacher extends BaseModel {
    public function create(int $uid,?string $no,array $subjects):void {
        $primary=$subjects[0];
        $this->db->prepare('INSERT INTO teachers(user_id,staff_no,subject_id,subject_code,year_level) VALUES(?,?,?,?,?)')->execute([$uid,$no?:null,$primary['id'],$primary['code'],$primary['year_level']]);
