...
final class Subject {
    ...
    public function registrationSubjectsForClass(string $className): array {
        if (!preg_match('/^([1-9])IT$/', strtoupper($className), $matches)) return [];
        $prefix = 'IT' . $matches[1] . '%';
        $statement = $this->db->prepare('SELECT id,code,name,year_level FROM subjects WHERE teacher_registration_enabled=1 AND UPPER(code) LIKE ? ORDER BY code');
